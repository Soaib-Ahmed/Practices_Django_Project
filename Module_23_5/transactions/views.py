...
def send_money_transfer_email(sender, receiver, amount, subject, template):
    message = render_to_string(template, {
        'sender': sender,
        'receiver': receiver,
        'amount': amount,
...
def send_money_received_email(sender, receiver, amount, subject, template):
    message = render_to_string(template, {
        'user': sender,
        'receiver': receiver,
        'amount': amount,
    })
    send_email = EmailMultiAlternatives(subject, '', to=[receiver.email])
    send_email.attach_alternative(message, "text/html")
    send_email.send()
...
class MoneyTransferView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        send_money_transfer_email(self.request.user, receiver, transfer.amount, "Money Transfer Message", "transactions/money_transfer_email.html")
        send_money_received_email(self.request.user, receiver, transfer.amount, "Money Received Message", "transactions/money_received_email.html")
